...
The repository leverages GitHub Actions to fetch prices over time. The purpose is just to collect stock data for potential later analysis.
...
The schedule is controlled by the `cron` line in `.github/workflows/collect.yml`. Cron format is `minute hour day month day-of-week`, all in **UTC**.

To pause collection entirely, comment out the `schedule:` block (keep `workflow_dispatch:` so you can still trigger it manually):
...
Reference cron patterns (pick one, don't paste more than one active `cron:` line unless you want multiple schedules):

| Goal | Cron | Approx. runs/month |
|---|---|---|
| Every 5 min, 07:00-20:00 UTC, weekdays | `*/5 7-20 * * 1-5` | ~1560 |
| Every 10 min, 07:00-20:00 UTC, weekdays | `*/10 7-20 * * 1-5` | ~780 |
| Every 15 min, 07:00-20:00 UTC, weekdays | `*/15 7-20 * * 1-5` | ~520 |
| Every 30 min, 07:00-20:00 UTC, weekdays (current) | `*/30 7-20 * * 1-5` | ~560 |
| Once per hour, weekdays | `0 7-20 * * 1-5` | ~280 |
| Twice a day (open + close), weekdays | `0 7,20 * * 1-5` | ~40 |
| Once daily at market close, weekdays | `0 20 * * 1-5` | ~20 |
| European session only, every 30 min, weekdays | `*/30 7-15 * * 1-5` | ~280 |
| US session only, every 30 min, weekdays | `*/30 13-20 * * 1-5` | ~280 |
| Every 30 min, all 7 days (e.g. if adding crypto) | `*/30 7-20 * * *` | ~784 |
