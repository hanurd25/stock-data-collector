...
This repo uses SSH for git operations. If you're setting this up fresh:
...
Copy the output and add it under GitHub **Settings → SSH and GPG keys → New SSH key**. Then point the remote at the SSH URL:
 
```bash
git remote set-url origin git@github.com:hanurd25/stock-data-collector.git
```
 
## .gitignore
 
This repo ignores local editor/environment clutter:
